    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        if self.enable_rev:
            ...
            for blk_id,blk in enumerate(self.blocks):
                ...
                x, thw = blk(x, thw)
                #print("out,x",blk_id, x.shape,thw)
                bbn_feat.append((x,thw))
            skip_feat = [bbn_feat[1],bbn_feat[7],bbn_feat[43]]
            ...
            if self.enable_detection:
                ...
                x = self.head(x)
        resized_skip_feat = []
        for feat, thw in skip_feat:
            t,h,w = thw 
            feat = feat[:,1:,:] # skip class token 
            #print("before reararnge",feat.shape,t,h,w,t*h*w)
            feat = rearrange(feat, 'b (t h w) c -> b c t h w', t=t,h=h, w=w)
            resized_skip_feat.append(feat)
            #print("after_resize",feat.shape)
        #exit(1)
        return x,resized_skip_feat
