...
class VideoDataset(Dataset):
    ...
    def __init__(self, frame_path, ann_path, semantic_masks, transform=None, clip_len=16, crop_size=224,
        ...
        print("label for classes", self.class_to_label)
        ...
        self.mask_path = Path('/home/c3-0/rmodi/datasets/JHMDB/puppet_mask/puppet_mask')
    ...
    def __getitem__(self, index):
        ...
        for id in range(keyframe,keyframe+self.K):
            ...
            frames.append(cv2.imread(str(self.frame_path/v_id/(str(id).zfill(5)+'.png'))))

        #read masks 

        mask_dir = self.mask_path/v_id/'puppet_mask.mat'
        mat = scipy.io.loadmat(mask_dir)
        mat = mat['part_mask']
        n_frames = mat.shape[2]
        masks_of_entire_video = [repeat(mat[:,:,i],'h w -> h w c',c=3) for i in range(n_frames)]

        masks = []
        for id in range(keyframe,keyframe+self.K):
            masks.append(masks_of_entire_video[id-1])
        
        # for id in range(keyframe,keyframe+self.K):
        #     # print(str(self.frame_path/v_id/(str(id).zfill(5)+'.jpg')))
        #     masks.append(cv2.imread(str(self.semantic_masks/self.mode/v_id/(str(id).zfill(5)+'.jpg'))))
        #threshold masks 
        masks = [(mask>0)*1 for mask in masks]


        # print("unqiue",np.unique(masks[0]))
        # print("lens",len(frames),len(masks))
...
def build_dataloader(cfg):
    ...
    train_dataset = VideoDataset( mode = 'train',\
                            frame_path = '/home/c3-0/rmodi/crcv_work/stream_lined_occlusions/models/MOC-Detector/data/JHMDB/Frames',\
                            ann_path = '/home/c3-0/rmodi/crcv_work/stream_lined_occlusions/models/MOC-Detector/data/JHMDB/JHMDB-GT.pkl',\
                            semantic_masks = None,\
...
if __name__ == "__main__":
    ...
    #check the mat file 
    # import scipy 
    # mat_path = '/home/c3-0/rmodi/datasets/JHMDB/puppet_mask/puppet_mask/catch/Ballfangen_catch_u_cm_np1_fr_goo_0/puppet_mask.mat'
    # mat = scipy.io.loadmat(mat_path)
    # print("loading mat file",type(mat['part_mask']),mat['part_mask'].shape)
    # mat = mat['part_mask']
    # n_frames = mat.shape[2]
    # masks = [repeat(mat[:,:,i],'h w -> h w c',c=3) for i in range(n_frames)]